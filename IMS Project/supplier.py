...
class supplierClass:
    def __init__(self,root):
       ...
       txt_empid=Entry(self.root,textvariable=self.var_sup_invoice,font=("goudy old style",15),bg="lightyellow").place(x=180,y=80,width=180)
       ...
       self.txt_desc.place(x=180,y=200,width=500,height=80)
...
#====================================================================#
    ...
    def delete(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                    messagebox.showerror("Error","Invoice Id Must be Required",parent=self.root)
            else:
                    cur.execute("Select * from supplier where invoice=?",(self.var_sup_invoice.get(),))
                    row=cur.fetchone()
                    if row==None:
